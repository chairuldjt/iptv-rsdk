package com.itops.iptvplayer.data.repository
...
import com.itops.iptvplayer.data.api.*
import com.itops.iptvplayer.data.cache.ChannelDao
import com.itops.iptvplayer.data.cache.ChannelEntity
import com.itops.iptvplayer.data.datastore.DataStoreManager
import com.itops.iptvplayer.data.parser.M3uParser
                ...
                if (body.status && body.data != null) {
                    ...
                    if (!newApiBaseUrl.isNullOrBlank()) {
                        ...
                        if (newApiBaseUrl != currentUrl) {
                            ...
                            if (testSuccess) {
                                ...
                                try {
                                    val oldService = com.itops.iptvplayer.data.api.RetrofitClient.getService(previousUrl)
                                    oldService.goodbye(com.itops.iptvplayer.data.api.GoodbyeRequest(device_id = dataStoreManager.getDeviceId()))
                                ...
                                try {
                                    val confirmService = com.itops.iptvplayer.data.api.RetrofitClient.getService(newApiBaseUrl)
                                    val deviceId = dataStoreManager.getDeviceId()
                                    val deviceName = getEffectiveDeviceName()
                                    confirmService.registerDevice(
                                        com.itops.iptvplayer.data.api.RegisterRequest(
                                            device_id = deviceId,
                                            device_name = deviceName,
                                            device_name_updated_at = dataStoreManager.getStbNameUpdatedAt(),
                                            app_version = BuildConfig.VERSION_NAME,
                    ...
                    if (eduPlaybackMode == "copy" && (eduSource == "web" || educationPath.isNotBlank())) {
                        ...
                        kotlinx.coroutines.CoroutineScope(Dispatchers.IO).launch {
                            com.itops.iptvplayer.util.EducationSyncManager.sync(context, forceSync = config.education_force_sync == true)
                        }
                    }
